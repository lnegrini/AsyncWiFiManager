...
AsyncWiFiManager.autoConnect("AP-NAME", "AP-PASSWORD");
```
if you just want an unsecured access point
```cpp
AsyncWiFiManager.autoConnect("AP-NAME");
```
...
```cpp
AsyncWiFiManager.autoConnect("AutoConnectAP", "password")
```
...
Before `autoConnect()`
```cpp
AsyncWiFiManager.setAPCallback(configModeCallback);
```
...
void loop() {
  ...
  if ( digitalRead(TRIGGER_PIN) == LOW ) {
    AsyncWiFiManager AsyncWiFiManager;
    AsyncWiFiManager.startConfigPortal("OnDemandAP");
...
#### Custom Parameters
You can use AsyncWiFiManager to collect more parameters than just SSID and password.
...
AsyncWiFiManager.setAPStaticIPConfig(IPAddress(10,0,1,1), IPAddress(10,0,1,1), IPAddress(255,255,255,0));
```
...
```cpp
AsyncWiFiManager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
...
```CPP
AsyncWiFiManager.setClass("invert"); // dark theme
AsyncWiFiManager.setScanDispPerc(true); // display percentages instead of graphs for RSSI
```
...
- If you would like to filter low signal quality networks you can tell AsyncWiFiManager to not show networks below an arbitrary quality %;
```cpp
AsyncWiFiManager.setMinimumSignalQuality(10);
```
...
```cpp
AsyncWiFiManager.setRemoveDuplicateAPs(false);
```
...
```CPP
AsyncWiFiManager AsyncWiFiManager(Serial1);
```
...
- pass in debug stream in constructor AsyncWiFiManager(Stream& consolePort)
